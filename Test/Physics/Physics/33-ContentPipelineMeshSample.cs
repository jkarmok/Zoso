...
namespace DigitalRune.Samples.Physics
{
  ...
  {
    // This sample uses the ModelWithCollisionMeshProcessor (see project "DigitalRune.Samples.Content.Pipeline"). 
      ...
      : base(game)
    {
      ...
      TriangleMeshShape triangleMesh = (TriangleMeshShape)originalModelNode.UserData;

      // We do not want to change the shared Saucer instance in the content manager.
      // Let's create a clone. But do not want to clone the shape in the UserData,
      // this would be a waste of time and memory.
      originalModelNode.UserData = null;
      _modelNode = originalModelNode.Clone();
      originalModelNode.UserData = triangleMesh;
