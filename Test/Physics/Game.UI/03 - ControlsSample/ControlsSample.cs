...
- How to add a custom text block control that displays the frame rate.",
  ...
  public class ControlsSample : Sample
  {
    ...
    private void CreateGui()
    {
      ...
      if (_themeNumber == 0)
      {
        _uiContentManager = new ContentManager(Services, "BlendBlueTheme");
        theme = _uiContentManager.Load<Theme>("Theme");
      }
      else if (_themeNumber == 1)
      {
        _uiContentManager = new ContentManager(Services, "AeroTheme");
        theme = _uiContentManager.Load<Theme>("Theme");
      }
      else if (_themeNumber == 2)
      {
        _uiContentManager = new ContentManager(Services, "NeoforceTheme");
        theme = _uiContentManager.Load<Theme>("ThemeRed");
      }
      else
      {
        _uiContentManager = new ContentManager(Services, "NeoforceTheme");
        theme = _uiContentManager.Load<Theme>("ThemeGreen");
      }
