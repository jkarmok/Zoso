...
namespace DigitalRune.Samples.Geometry
{
  ...
  public static class CharacterControllerLevel
  {
    public static void Load(CollisionDomain collisionDomain)
    {
      ...
      // The height field lies in the x/z plane. We make it 20 units wide.
      heightField.WidthX = 20;
      heightField.WidthZ = 20;
      // The heights are stored in simple float array.
      ...
      for (int row = 0; row < heightField.Array.GetLength(0); row++)
      {
        for (int column = 0; column < heightField.Array.GetLength(1); column++)
        {
          ...
          else
          {
            ...
            heightField.Array[row, column] = 1 + (float)(Math.Cos(column / 2f) * Math.Sin(row / 2f) * 1);
          }
        }
      }
