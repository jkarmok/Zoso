...
namespace DigitalRune.Samples.Particles
{
  ...
  {
    // Checks for common configuration errors and writes a message to the Console.
    [Conditional("DEBUG")]
    public static void Validate(ParticleSystem particleSystem)
    {
      ...
      {
        Console.WriteLine("----- \"{0}\" (type {1}):", particleSystem.Name, particleSystem.GetType().Name);
    ...
    public static string CheckForUninitializedEffectorProperties(ParticleSystem particleSystem)
    {
      ...
      {
        foreach (var propertyInfo in effector.GetType().GetProperties())
        {
        ...
        foreach (var propertyInfo in effector.GetType().GetProperties())
        {
          ...
          if (particleParameterAttribute != null)
          {
            ...
            if (particleSystem.Parameters.Contains(parameterName))
              continue;                // Parameter was found in the particle systsem.
