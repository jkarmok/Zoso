...
parameter that are currently supported in hardware instancing are:
  ...
  ""InstanceAlpha"" (float),
  ""SceneNodeType"" (float)
InstanceColor and SceneNodeType are mutually exclusive - only one of them can be used at a time. 
Automatic support for other per-instance parameters is planned for future updates.",
    8)]
