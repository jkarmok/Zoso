  ...
  public class VehicleObject : GameObject
  {
    ...
    private void UpdateSteeringAngle(float deltaTime)
    {
      ...
      if (_inputService.IsDown(Keys.D))
        direction -= 1;
      direction -= _inputService.GetGamePadState(LogicalPlayerIndex.One).ThumbSticks.Left.X;
    ...
    private void UpdateAcceleration(float deltaTime)
    {
      ...
      if (_inputService.IsDown(Keys.S))
        direction -= 1;
      direction += _inputService.GetGamePadState(LogicalPlayerIndex.One).Triggers.Right - _inputService.GetGamePadState(LogicalPlayerIndex.One).Triggers.Left;
