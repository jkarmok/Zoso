...
using DigitalRune.Samples.Animation;
...
Since the skeleton of the Dude model is very different from a usual Kinect player skeleton,
...
as the Kinect player skeleton. The constraint parameters (MaxForce, ErrorReduction, Softness)
of the marionette constraints and the ragdoll constraints could also deserve more tweaking.
  ...
  class KinectRagdollMarionetteSample : BasicSample
  {
    ...
    private void DrawDebugInfo()
    {
      ...
      if (!_kinectWrapper.IsRunning)
      {
        ...
        debugRenderer.DrawText("\n\nCOULD NOT INITIALIZE KINECT DEVICE! PLEASE MAKE SURE THE DEVICE IS CONNECTED.");
        return;
      }
