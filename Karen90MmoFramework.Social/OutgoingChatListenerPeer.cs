...
namespace Karen90MmoFramework.Server.Social
{
	public sealed class OutgoingChatListenerPeer : IChatManager, IOutgoingServerPeer
	{
		...
		public ListenerState ListenerState
		{
			get
			{
				return (ListenerState) this.listenerState;
			}

			private set
			{
				Interlocked.Exchange(ref this.listenerState, (int) value);
			}
		}
		...
		private bool IsShuttingDown
		{
			get
			{
				return this.shuttingDown > 0;
			}

			set
			{
				Interlocked.Exchange(ref this.shuttingDown, value ? 1 : 0);
			}
		}
		...
		public void Disconnect()
		{
			...
			if (this.outgoingServerPeer != null)
		...
		private void AcquireChatServerConnection(long messageDelay = 0)
		{
			SubServerApplication.Instance.QueueOutgoingMasterServerOperationRequest(
				...
				(o, r) => this.HandleOperationResponseAcquireServerResponse(SocialApplication.Instance.MasterPeer.Protocol, r), messageDelay);
		}
		...
		private void HandleOperationResponseAcquireServerResponse(IRpcProtocol protocol, OperationResponse operationResponse)
		{
			...
			Interlocked.CompareExchange(ref this.listenerState, (int) ListenerState.Connecting, (int) ListenerState.Disconnected);

			var newListenerState = this.ListenerState;
			...
			SocialApplication.Instance.ConnectToExternalServer(new IPEndPoint(ipAddress, port), "Chat", this);
		}
		...
		public void CreateChannel(string name, ChannelType channelType, ChannelCreatedCallback callback)
		{
			...
			lock (this.channelCreatedCallbacks)
		...
		void IOutgoingServerPeer.OnConnect(ServerPeerBase connectedServerPeer)
		{
			...
			{
				foreach (var pendingOperation in this.pendingOperations)
		...
		private void HandleOperationResponseCreateChatChannel(IRpcProtocol protocol, OperationResponse operationResponse)
		{
			...
			lock (this.channelCreatedCallbacks)
				if (this.channelCreatedCallbacks.TryGetValue(response.CallbackId, out callback))
					this.channelCreatedCallbacks.Remove(response.CallbackId);

			if (callback == null)
			{
				_logger.DebugFormat("Received a {0} which does not have an EventHandler", response.GetType());
				return;
			}
		...
		{
			if (this.ListenerState == ListenerState.Connected)
			{
				this.outgoingServerPeer.SendOperationRequest(operationRequest, new SendParameters());
			}
			else
			{
				lock (this.pendingOperations)
